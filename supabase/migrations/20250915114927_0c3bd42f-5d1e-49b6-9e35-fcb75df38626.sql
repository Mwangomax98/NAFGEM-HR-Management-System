...
-- 2. Delete trip requests first (before drivers/vehicles)
DELETE FROM public.trip_requests;
...
-- 3. Delete demo KPI and project data
DELETE FROM public.kpi_values;
DELETE FROM public.weekly_targets;
DELETE FROM public.kpis;

-- 4. Delete demo vehicle and driver data (now safe to delete)
...
-- 5. Delete demo employee profiles (keep admin if exists)
DELETE FROM public.employee_profiles WHERE user_id IN (
  SELECT p.id FROM public.profiles p 
  WHERE p.email != 'itofficernafgem@gmail.com'
);

-- 6. Clean up security events (keep recent admin events)
DELETE FROM public.security_events WHERE user_id IN (
  SELECT p.id FROM public.profiles p 
  WHERE p.email != 'itofficernafgem@gmail.com'
) OR created_at < (now() - interval '7 days');

-- 7. Delete demo user roles (keep admin role)
DELETE FROM public.user_roles WHERE user_id IN (
  SELECT p.id FROM public.profiles p 
  WHERE p.email != 'itofficernafgem@gmail.com'
);

-- 8. Delete demo profiles (keep admin profile)
