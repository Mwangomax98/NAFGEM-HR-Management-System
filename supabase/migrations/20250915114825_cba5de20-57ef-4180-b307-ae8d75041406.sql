-- Clean up all demo data from the system (fixed to skip views)
-- Keep only the main admin user for system access

-- 1. Delete demo operational data in correct order (respecting foreign keys)
...
DELETE FROM public.task_evaluations WHERE evaluator_id IN (
  SELECT p.id FROM public.profiles p 
  WHERE p.email != 'itofficernafgem@gmail.com'
);

DELETE FROM public.task_submissions WHERE weekly_task_id IN (
  SELECT wt.id FROM public.weekly_tasks wt
  JOIN public.profiles p ON wt.employee_id = p.id
  WHERE p.email != 'itofficernafgem@gmail.com'
);
...
DELETE FROM public.timesheet_attachments WHERE uploaded_by IN (
  SELECT p.id FROM public.profiles p 
  WHERE p.email != 'itofficernafgem@gmail.com'
);
...
DELETE FROM public.exit_conversations WHERE sender_id IN (
  SELECT p.id FROM public.profiles p 
  WHERE p.email != 'itofficernafgem@gmail.com'
);

DELETE FROM public.exit_attachments WHERE uploaded_by IN (
  SELECT p.id FROM public.profiles p 
  WHERE p.email != 'itofficernafgem@gmail.com'
);

DELETE FROM public.exit_assets WHERE exit_request_id IN (
  SELECT er.id FROM public.exit_requests er
  JOIN public.profiles p ON er.employee_id = p.id
  WHERE p.email != 'itofficernafgem@gmail.com'
);
...
-- 2. Delete demo KPI data (skip views, only delete from actual tables)
