...
// Utility function to export timesheet data as PDF
export const exportTimesheetToPDF = (timesheet: any, entries: any[]) => {
  const printWindow = window.open('', '_blank');
  ...
  const html = `
    ...
    <html>
    <head>
      <title>Timesheet - ${format(new Date(timesheet.week_start_date), 'MMM dd')} - ${format(new Date(timesheet.week_end_date), 'MMM dd, yyyy')}</title>
      <style>
        body {
          font-family: Arial, sans-serif;
          margin: 20px;
    ...
    <body>
      <div class="header">
        <h1>TIMESHEET</h1>
        <h2>${format(new Date(timesheet.week_start_date), 'MMMM dd')} - ${format(new Date(timesheet.week_end_date), 'MMMM dd, yyyy')}</h2>
      </div>

      <div class="timesheet-header">
        <p><strong>Status:</strong> <span class="badge status-${timesheet.status}">${timesheet.status.toUpperCase()}</span></p>
        ${timesheet.submitted_at ? `<p><strong>Submitted:</strong> ${new Date(timesheet.submitted_at).toLocaleDateString()}</p>` : ''}
        ${timesheet.approved_by ? `<p><strong>Approved By:</strong> ${timesheet.approved_by}</p>` : ''}
      </div>

      <div class="summary-grid">
        <div class="summary-card">
          <div class="label">Total Hours</div>
          <div class="value">${timesheet.total_hours}h</div>
        </div>
        <div class="summary-card">
          <div class="label">Overtime Hours</div>
          <div class="value">${timesheet.overtime_hours}h</div>
        </div>
        <div class="summary-card">
        ...
        <div class="summary-card">
          <div class="label">Week Days</div>
          <div class="value">${entries.filter((e: any) => new Date(e.entry_date).getDay() !== 0 && new Date(e.entry_date).getDay() !== 6).length}</div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <table>
          ...
          <tbody>
            ${entries.map(entry => `
              <tr class="entry-row">
                ...
                <td>${format(new Date(entry.entry_date), 'EEEE')}</td>
          ...
          <tfoot>
            <tr style="background: #007580; color: white; font-weight: bold;">
              <td colspan="3" style="text-align: right;">TOTAL:</td>
              <td><strong>${timesheet.total_hours}h</strong></td>
              <td></td>
            </tr>
          </tfoot>
        </table>
